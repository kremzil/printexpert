...
import { TAGS, calculatorTag } from "@/lib/cache-tags"
...
export async function getProductCalculatorData({
  ...
  return unstable_cache(
    async (): Promise<CalculatorData | null> => {
      return buildCalculatorDataFromPricingModels(productId)
    },
    ["calculator-data", productId],
    { tags: [calculatorTag(productId), TAGS.CALCULATORS], revalidate: 3600 }
  )()
}
