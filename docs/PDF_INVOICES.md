...
## Cenová ponuka (Предварительный расчёт)
...
**Доступ:** Все пользователи (предназначено для B2B)

### Содержимое PDF

- **Шапка:** Логотип, данные компании
- **Информация о понуке:** Номер, дата создания, срок действия (14 дней)
- **Таблица товаров:**
  - Название товара
  - Конфигурация (атрибуты из калькулятора)
  - Размеры (если указаны)
  - Количество
  - Цена за единицу
  - Итого по позиции
- **Итоги:** Сумма без DPH, DPH, Итого
- **Примечание:** Информация о сроке действия и незавязанности

### UI

Кнопка "Cenová ponuka" в корзине (только для B2B режима):

```
[ Download icon ] Cenová ponuka
```

### Модуль

- **Шаблон:** `lib/pdf/quote-template.tsx`
- **Генерация:** `lib/pdf/generate-quote.ts`
- **Типы:** `QuoteData`, `QuoteItem`, `QuoteInfo`, `QuoteTotals` в `lib/pdf/types.ts`

## Следующие шаги

- [x] ~~Генерация расчёта стоимости корзины (quote/estimate)~~ — реализовано
