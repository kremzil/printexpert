...
## Единая цена для фида и карточек товара

Чтобы избежать расхождений между маркетинговым фидом и витриной, используется единая серверная логика вычисления цены:

- общий резолвер: `resolveFeedProductPrice(...)` в `lib/feeds/catalog-feed.ts`;
- фиды `/feeds/google.xml` и `/feeds/meta.csv` используют этот резолвер через `getCatalogFeedData(...)`;
- карточки товара (`ProductCard`) получают заранее рассчитанное поле `feedPrice` из `lib/catalog.ts` и используют его как fallback.

Приоритет отображения цены в карточке:

1. скидочная цена (`priceAfterDiscountFrom`), если она валидна и меньше `priceFrom`;
2. базовая цена (`priceFrom`), если валидна;
3. вычисленная сервером `feedPrice` (тот же алгоритм, что в фиде);
4. если цены нет — `Na vyžiadanie`.

Это гарантирует, что цена в карточке и в товарном фиде считается одинаково при отсутствии ручных полей цены.
