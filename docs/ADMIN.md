...
Поведение:
- При обновлении/создании сбрасываются теги кэша:
  - `categories` и `category:*` (внутренние теги витрины),
  - `nav-data` и `catalog-data` (меню и категории),
  - `top-products` (если выбранный товар относится к изменённой категории).
...
Ручная регенерация кэша меню и категорий (теги `nav-data` и `catalog-data`).
