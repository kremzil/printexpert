...
- Новые флаги видимости: `showInB2b` и `showInB2c`. В карточке товара добавлены чекбоксы "Zobraziť v režimoch" (B2B / B2C), по умолчанию оба включены.
- `ProductTitleEditor` теперь рендерится внутри формы сохранения, чтобы скрытые поля `name`/`slug` попадали в submit и корректно сохранялись.
