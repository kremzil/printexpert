...
- текущий B2B lead-flow реализован через `POST /api/quote-request`:
  - клиент хранит список dopytu в `localStorage` только для UX (не источник истины)
  - сервер валидирует payload (`zod`), применяет honeypot + rate limit и отправляет письмо менеджеру по SMTP
  - для товаров из конфигуратора передаются выбранные параметры (množstvo/rozmery/voľby/cena) как часть запроса
...
- [x] B2B lead endpoint `POST /api/quote-request` с anti-spam защитой (honeypot + rate limit)
