...
- Логика цены в карточке товара синхронизирована с фидом:
  - общий серверный резолвер `resolveFeedProductPrice(...)`;
  - приоритет в карточке: `priceAfterDiscountFrom` (если это реальная скидка) → `priceFrom` → `feedPrice` (fallback по правилам фида);
  - это убирает расхождения цены между `/feeds/google.xml`, `/feeds/meta.csv` и карточками на витрине.
